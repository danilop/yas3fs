...
class SNS_HTTPRequestHandler(BaseHTTPServer.BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if signature_version != '1':
            logging.debug('unknown signature version')
            self.send_response(404)
            return
...
class YAS3FS(LoggingMixIn, Operations):
    ...
    def __init__(self, options):
        ...
        if self.sns_topic_arn:
            logging.info("AWS region for SNS/SQS: '" + self.aws_region + "'")
        ...
        self.queue_wait_time = options.queue_wait_time
        self.queue_polling_interval = options.queue_polling_interval
        ...
        self.max_num_entries = options.cache_max_num_entries
        logging.info("Cache max entries: '%i'" % self.max_num_entries)
        self.max_props_size = options.cache_max_props_size * (1024 * 1024) # To convert MB to bytes
        logging.info("Cache max size (in bytes): '%i'" % self.max_props_size)
        self.check_memory_interval = options.cache_check_interval # seconds
        logging.info("Cache check memory interval (in seconds): '%i'" % self.check_memory_interval)
        ...
        if self.hostname:
            logging.info("Hostname to listen to SNS HTTP notifications: '%s'" % self.hostname)
    ...
    def check_memory_usage(self):
        while True:
            ...
            logging.debug("num_entries, props_size: %i, %i" % (num_entries, props_size))
            if num_entries > self.max_num_entries or props_size > self.max_props_size:
                with self.cache.lock:
                    path = self.cache.lru.popleft()
                    logging.debug("purge: %s ?" % path)
                    if self.cache.get(path, 'open') or self.cache.has(path, 'change'):
                        logging.debug("purge: no")
                        logging.debug("open: %i" % self.cache.get(path, 'open'))
                        logging.debug("change: %s" % self.cache.has(path, 'change'))
                        self.cache.lru.append(path)
                    else:
                        logging.debug("purge: yes")
