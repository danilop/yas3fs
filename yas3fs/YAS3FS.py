...
from .HTTPServer import SNS_HTTPServer, SNS_HTTPRequestHandler
...
class UTF8DecodingKey(boto.s3.key.Key):
    ...
    def compute_md5(self, fp, size=None):
        hex_digest, b64_digest, data_size = boto.utils.compute_md5(
            fp, buf_size=131072, size=size)
...
class YAS3FS(LoggingMixIn, Operations):
    ...
    def __init__(self, options):
        ...
        # Maximum number of S3 retries (outside of boto)
        self.s3_retries = options.s3_retries
        ...
        self.yas3fs_xattrs = ['user.yas3fs.bucket', 'user.yas3fs.key',
                              'user.yas3fs.URL', 'user.yas3fs.signedURL', 'user.yas3fs.expiration']
        ...
        if s3url.scheme != 's3':
            utils.error_and_exit(
                "The S3 path to mount must be in URL format: s3://BUCKET/PATH")
        ...
        if self.s3_bucket_name == '':
            utils.error_and_exit("The S3 bucket cannot be empty")
        self.sns_topic_arn = options.topic
        if self.sns_topic_arn:
            logger.info("AWS region for SNS and SQS: '" +
                        self.aws_region + "'")
        ...
        logger.info("AWS Managed Encryption enabled: %s" %
                    self.aws_managed_encryption)
        ...
        if options.use_ec2_hostname:
            # Very slow (to fail) outside of EC2
        ...
        if self.hostname:
            logger.info(
                "Public hostname to listen to SNS HTTP notifications: '%s'" % self.hostname)
        self.sns_http_port = int(options.port or '0')
        if options.port:
            logger.info(
                "TCP port to listen to SNS HTTP notifications: '%i'" % self.sns_http_port)
        self.s3_num = options.s3_num
        logger.info(
            "Number of parallel S3 threads (0 to disable writeback): '%i'" % self.s3_num)
        self.download_num = options.download_num
        logger.info("Number of parallel downloading threads: '%i'" %
                    self.download_num)

        # for https://github.com/danilop/yas3fs/issues/46
        self.download_retries_num = options.download_retries_num
        logger.info("Number download retry attempts: '%i'" %
                    self.download_retries_num)
        self.download_retries_sleep = options.download_retries_sleep
        logger.info("Download retry sleep time seconds: '%i'" %
                    self.download_retries_sleep)
        ...
        logger.info("Read retry sleep time seconds: '%i'" %
                    self.read_retries_sleep)

        self.prefetch_num = options.prefetch_num
        logger.info("Number of parallel prefetching threads: '%i'" %
                    self.prefetch_num)
        self.buffer_size = options.buffer_size * 1024  # To convert KB to bytes
        logger.info(
            "Download buffer size (in KB, 0 to disable buffering): '%i'" % self.buffer_size)
        self.buffer_prefetch = options.buffer_prefetch
        logger.info("Number of buffers to prefetch: '%i'" %
                    self.buffer_prefetch)
        self.write_metadata = not options.no_metadata
        logger.info("Write metadata (file system attr/xattr) on S3: '%s'" %
                    str(self.write_metadata))
        self.full_prefetch = options.prefetch
        logger.info("Download prefetch: '%s'" % str(self.full_prefetch))
        self.multipart_size = options.mp_size * \
            (1024 * 1024)  # To convert MB to bytes
        logger.info("Multipart size: '%s'" % str(self.multipart_size))
        self.multipart_num = options.mp_num
        logger.info("Multipart maximum number of parallel threads: '%s'" %
                    str(self.multipart_num))
        self.multipart_retries = options.mp_retries
        logger.info("Multipart maximum number of retries per part: '%s'" %
                    str(self.multipart_retries))
        self.default_expiration = options.expiration
        logger.info("Default expiration for signed URLs via xattrs: '%s'" %
                    str(self.default_expiration))
        ...
        # AWS Initialization
        if self.aws_region not in (r.name for r in boto.s3.regions()):
            utils.error_and_exit(
                "wrong AWS region '%s' for S3" % self.aws_region)
        try:
            s3kw = {
            ...
            utils.error_and_exit("no AWS credentials found")
        if not self.s3:
            utils.error_and_exit("no S3 connection")
        try:
        ...
        if self.sns_topic_arn:
            if self.aws_region not in (r.name for r in boto.sns.regions()):
                utils.error_and_exit(
                    "wrong AWS region '%s' for SNS" % self.aws_region)
        ...
        if self.sqs_queue_name or self.new_queue:
            ...
            if not self.sns_topic_arn:
                utils.error_and_exit(
                    "The SNS topic must be provided when an SQS queue is used")
            if self.aws_region not in (r.name for r in boto.sqs.regions()):
                utils.error_and_exit(
                    "wrong AWS region '" + self.aws_region + "' for SQS")
            ...
            if self.new_queue:
                ...
                self.sqs_queue_name = '-'.join(['yas3fs',
                                                ...
                                                hostname,
                                                self.unique_id])
                # fix for https://github.com/danilop/yas3fs/issues/40
                self.sqs_queue_name = self.sqs_queue_name[:80]
                self.sqs_queue_name = re.sub(r'-+', '-', self.sqs_queue_name)
                logger.info("Attempting to create SQS queue: " +
                            self.sqs_queue_name)
            ...
            if not self.queue:
                try:
                    ...
                    utils.error_and_exit(
                        "Unexpected error creating SQS queue:" + str(sqsErr))
        ...
        if self.multipart_size < 5242880:
            utils.error_and_exit(
                "The minimum size for multipart upload supported by S3 is 5MB")
        if self.multipart_retries < 1:
            utils.error_and_exit(
                "The number of retries for multipart uploads cannot be less than 1")

        self.plugin = None
        if (options.with_plugin_file):
            self.plugin = YAS3FSPlugin.load_from_file(
                self, options.with_plugin_file, options.with_plugin_class)
        elif (options.with_plugin_class):
            self.plugin = YAS3FSPlugin.load_from_class(
                self, options.with_plugin_class)
    ...
    def check_threads(self, first=False):
        ...
        if self.sns_topic_arn:
            if utils.thread_is_not_alive(self.publish_thread):
                logger.debug("%s publish thread" % display)
                self.publish_thread = TracebackLoggingThread(
                    target=self.publish_messages, name="SNSPublisher")
                self.publish_thread.daemon = True
                self.publish_thread.start()

        if self.sqs_queue_name:
            if utils.thread_is_not_alive(self.queue_listen_thread):
                logger.debug("%s queue listen thread" % display)
                self.queue_listen_thread = TracebackLoggingThread(
                    target=self.listen_for_messages_over_sqs, name="SQSListener")
                self.queue_listen_thread.daemon = True
                self.queue_listen_thread.start()

        if self.sns_http_port:
            if utils.thread_is_not_alive(self.http_listen_thread):
                logger.debug("%s HTTP listen thread" % display)
                self.http_listen_thread = TracebackLoggingThread(
                    target=self.listen_for_messages_over_http, name="HTTPListener")
                self.http_listen_thread.daemon = True
                self.http_listen_thread.start()

        if utils.thread_is_not_alive(self.check_cache_thread):
            logger.debug("%s check cache thread" % display)
            self.check_cache_thread = TracebackLoggingThread(
                target=self.check_cache_size, name="CacheChecker")
    ...
    def init(self, path):
        ...
        self.check_status_thread = TracebackLoggingThread(
            target=self.check_status, name="StatusChecker")
        self.check_status_thread.daemon = True
        self.check_status_thread.start()

        if self.sqs_queue_name:
            logger.debug("Subscribing '%s' to '%s'" %
                         (self.sqs_queue_name, self.sns_topic_arn))
            response = self.sns.subscribe_sqs_queue(
                self.sns_topic_arn, self.queue)
        ...
        if self.sns_http_port:
            self.http_listen_thread = None
            self.sns.subscribe(self.sns_topic_arn, 'http',
                               self.http_listen_url)
        ...
        while self.sqs_queue_name:
            if self.queue_wait_time > 0:
                ...
                messages = self.queue.get_messages(
                    10, wait_time_seconds=self.queue_wait_time)
            else:
    ...
    def process_message(self, messages):
        ...
        if c[1] in ('mkdir', 'mknod', 'symlink') and c[2] is not None:
            self.delete_cache(c[2])
        elif c[1] in ('rmdir', 'unlink') and c[2] is not None:
            self.delete_cache(c[2])
        elif c[1] == 'rename' and c[2] is not None and c[3] is not None:
            self.delete_cache(c[2])
            self.delete_cache(c[3])
        elif c[1] == 'upload':
            # fix for https://github.com/danilop/yas3fs/issues/42
            if c[2] is not None and len(c) == 4:
    ...
    def process_native_s3_event(self, event):
        ...
        if user_id == self.current_user_principalId:
            logger.debug("Native S3 event %s on %s from current yas3fs user %s discarded" % (
                event_kind, path, user_id))
            return

        logger.info("Native S3 event %s on %s by %s. Deleting cache for %s" % (
            event_kind, path, user_id, path))
    ...
    def publish_status(self):
        ...
        message = ['status', hostname, num_entries,
                   mem_size, disk_size, dq, pq, s3q]
    ...
    def publish_messages(self):
        while self.sns_topic_arn:
            try:
                ...
                self.sns.publish(self.sns_topic_arn,
    ...
    def check_status(self):
        ...
        while self.cache_entries:
            ...
            logger.info("multipart_uploads_in_progress = " +
                        str(self.multipart_uploads_in_progress))
    ...
    def check_cache_size(self):
        ...
        while self.cache_entries:
            num_entries, mem_size, disk_size = self.cache.get_memory_usage()
            logger.debug("check_cache_size get_memory_usage() -> num_entries=%r mem_size=%r disk_size=%r",
                         num_entries, mem_size, disk_size)
    ...
    def reset_parent_readdir(self, path):
        ...
        (parent_path, dir) = os.path.split(path)
        logger.debug("reset_parent_readdir '%s' parent_path '%s'" %
                     (path, parent_path))
    ...
    def get_metadata(self, path, metadata_name, key=None):
        ...
        with self.cache.get_lock(path):
            ...
            logger.debug("get_metadata <- '%s' '%s' '%r' '%s'" %
                         (path, metadata_name, key, metadata_values))
        ...
        try:
            if action == 'delete':
                ...
                if full_size != uploaded_key.size:
                    logger.error("Multipart-upload Key Sizes do not match for '%r' local: %i remote: %i" %
                                 (key, full_size, uploaded_key.size))
    ...
    def rename_on_s3(self, key, source_path, target_path, dir):
        logger.debug("rename_on_s3 '%s' '%s' -> '%s' dir?%s" %
    ...
    def open(self, path, flags):
        ...
        with self.cache.get_lock(path):
            ...
            logger.debug("open '%s' '%i' '%s'" %
                         (path, flags, self.cache.get(path, 'data').get('open')))
        return 0
    ...
    def write(self, path, new_data, offset, fh=None):
        ...
        if sys.version_info < (3, ):
            if isinstance(new_data, unicode):  # Fix for unicode
                logger.debug("write '%s' '%i' '%i' '%s' unicode fix" %
                             (path, len(new_data), offset, fh))
        ...
        with data.get_lock():
            if not data.content:
                logger.info("write awake '%s' '%i' '%i' '%s' no content" %
                            (path, len(new_data), offset, fh))
                return 0
            ...
            logger.debug("write '%s' '%i' '%i' '%s' '%s' content" %
                         (path, len(new_data), offset, fh, contentname))
            data.content.seek(offset)
